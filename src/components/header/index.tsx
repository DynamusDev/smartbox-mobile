...
import { Container, Title, Space, Image } from './styles';
    ...
    <Container style={{ backgroundColor: props.bgColor || '#1e111d' }} >
      {
        props.leftIcon
          ...
          : props.avatarLeft ? <Image resizeMode='contain' source={{ uri: props.avatar }} /> : <Space />
      }
      <Title style={{ color: props.color || '#eb6726' }}> {translate(props.tx) || props.title} </Title>
      {
        props.rightIcon
          ? <Feather name={props.rightIcon} color={props.iconColor || props.color} size={25} onPress={props.onRightPress} />
          : props.avatarRight ? <Image resizeMode='contain' source={{ uri: props.avatar }} /> : <Space />
      }
    </Container>
  )
}